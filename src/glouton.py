...
class Glouton:
        ...
        for (o, a) in self.B:
            for g in self.goals:
                ...
                if (self.isInGoal((o, a), (g["posts"], g["direction"]))):
                    ...
                    if (o[0], o[1]) not in self.OppCoord :
                        self.OppCoord.append((o[0], o[1]))
                else:
    ...
    def creatingGraph(self):
        ...
        self.OppNodes = list(self.G.nodes)
    ...
    def defenderMaxDegree(self, G, markedNodes=[]):
        maxDegree = 0
        nodeMaxDegree = None
        nodes = list(G.nodes)
        for node in nodes:
            if node not in markedNodes:
                ...
                for neighbour in G.neighbors(node):
                    if neighbour not in markedNodes:
    ...
    def glouton(self, G, Bc, k):
        dominatingSet = []
        markedNodes = []
        i = 0
        while i < k:
            i = 1 + i
            nodeMaxDegree = self.defenderMaxDegree(G, markedNodes)
            if nodeMaxDegree is None:
                return []
            markedNodes.append(nodeMaxDegree)
            dominatingSet.append(nodeMaxDegree)
            for neighbourNode in G.neighbors(nodeMaxDegree):
                markedNodes.append(neighbourNode)
            print(dominatingSet)
            if self.isDominated(G, Bc, dominatingSet):
...
if __name__ == "__main__":
    ...
    filename =  sys.argv[1]
    Glouton = Glouton(filename)
    Glouton.saveJSon(sys.argv[2])
